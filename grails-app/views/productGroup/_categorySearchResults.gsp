...
<g:each in="${categories}" var="category" status="i">
    <div id="category-result-${i + 1}" class="row ml-0 mr-0 pt-2 pb-2 wl-striped${i % 2}">
        ...
        <a id="category-result-${i + 1}-select-button" href="#" class="col-1 btn btn-wl my-auto"
           onclick="addProductGroupCategoryProducts(${category.id})" data-dismiss="modal">Select</a>
    </div>
</g:each>

<div class="my-3 text-right">
    <util:remotePaginate action="categorySearch" total="${totalResults ?: 0}" update="category-search-results"
                         offset="${offset ?: 0}" max="${max ?: 50}"
                         params="[searchTerm: searchTerm, searchBy: searchBy]"/>
</div>
<div class="row mt-5 pb-2 ml-0 mr-0 table-wl bottom-border">
    ...
    <g:if test="${!userColumns || userColumns?.columns?.find { it.column == "reasonOther" }?.enabled}">
        <div class="col">Reason Other</div>
    </g:if>
</div>
        ...
        <div class="row ml-0 mr-0 pt-2 pb-2 wl-striped${i%2}">
            ...
            <g:if test="${!userColumns || userColumns?.columns?.find { it.column == "reason" }?.enabled}">
                <div class="col my-auto">
                    ...
                    <g:elseif test="${tenderMovement.type.equals('PAID_OUT')}"><g:message code="PaidOutReason.${tenderMovement.reason}" /></g:elseif>
            ...
            <g:if test="${!userColumns || userColumns?.columns?.find { it.column == "reasonOther" }?.enabled}">
                <div class="col my-auto">
                    <g:if test="${!tenderMovement.reasonOther}">N/A</g:if>
                    <g:else>${tenderMovement.reasonOther}</g:else>
                </div>
            </g:if>
            %{--            <g:if test="${!userColumns || userColumns?.columns?.find { it.column == "usersName" }?.enabled}">--}%
            %{--                <div class="col my-auto">${tenderMovement.usersName}</div>--}%
            %{--            </g:if>--}%
        </div>
    </g:each>
</div>

...
<div class="row mt-3 mb-2">
    <div class="col-8 pr-0" style="-ms-flex: 0 0 63%; flex: 0 0 63%; max-width: 63%;">
        <g:if test="${shift.onHoldReconciliationTotals.size() > 0}">
            ...
            <div id="cashUpContainer" class="mt-3 mr-4">
                <g:render template="cashUpByTotals" model="[values: [ cashTotal: (shift.onHoldReconciliationTotals.find { it.tenderType.name() == 'CASH' }?.value ?: BigDecimal.ZERO), vouchersTotal: shift.onHoldReconciliationTotals.find { it.tenderType.name() == 'VOUCHER' }?.value ?: BigDecimal.ZERO ]]" />
            </div>
        </g:if>
        <g:else>
            <div class="row cash-up-by">

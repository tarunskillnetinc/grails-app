<g:each in="${topLevelCategories}" var="category" status="i">
    <g:if test="${matchedCategories == null || matchedCategories?.contains(category.id)}">
        <div id="category-result-${i+1}" class="row ml-0 mr-0 pt-2 pb-2 wl-striped${row++%2} hoverable" title="Click to edit." style="cursor: pointer;" onclick="document.location.href='${createLink(action:'show', id: category.id)}';">
            ...
            <g:if test="${!userColumns || userColumns?.columns?.find { it.column == "retailerCategoryCode" }?.enabled}">
                <div id="category-result-${i+1}-retailer-Category-Code" class="col-1 text-truncate">${category.retailerCategoryCode}</div>
            </g:if>

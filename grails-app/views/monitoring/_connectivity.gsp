...
<div id="search-results">
    ...
    <g:each in="${rabbitQueues}" var="rabbitQueue" status="i">
        <div class="row ml-0 mr-0 pt-2 pb-2 wl-striped${i%2} hoverable">
            <div class="col-2 my-auto">${rabbitQueue.storeId}</div>
            <div class="col-2 my-auto">${rabbitQueue.tillId}</div>
            <div class="col-2 my-auto">${rabbitQueue.messages}</div>
            <div class="col-2 my-auto"><g:formatDate format="dd/MM/yyyy HH:mm:ss" date="${rabbitQueue?.idle_since?.toDate() ?: new Date()}" /></div>
            ...
            <div class="col-2 my-auto text-right">
                <button class="btn btn-wl" onclick="purgeQueue(${rabbitQueue.storeId}, ${rabbitQueue.tillId})">Clear</button>
                &nbsp;
                <button class="btn btn-danger" onclick="deleteQueue(${rabbitQueue.storeId}, ${rabbitQueue.tillId})">Delete</button>
            </div>
        </div>
    </g:each>
</div>
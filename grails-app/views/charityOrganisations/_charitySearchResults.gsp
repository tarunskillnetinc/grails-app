...
<div class="row mt-5 pb-2 ml-0 mr-0 table-wl bottom-border">
    ...
    <div class="col-2 font-weight-bold">
        <a id="charity-header-customer-reference" href="#" onclick="searchCharity({ max: ${sortParams?.max}, offset: ${sortParams?.offset}, sortColumn: 'active', sortOrder: ${sortParams?.sortColumn == 'active' ? sortParams?.sortOrder == 'asc' ? '\'desc\'' : '\'asc\'' : '\'asc\''} });">
            Status
        </a>
    </div>
</div>
...
<div id="search-results">
    ...
    <g:each in="${charities}" var="charity" status="i">
        <div id="supplier-result-${i+1}" class="row ml-0 mr-0 pt-2 pb-2 wl-striped${i%2} hoverable" style="cursor: pointer;" onclick="editSupplier(${charity.id});">
            <div id="supplier-result-${i+1}-reference" class="col-3 my-auto" style='word-break: break-all; word-wrap: break-word;'>${charity.organisationName}</div>
            <div id="supplier-result-${i+1}-name" class="col-3 my-auto" style='word-break: break-all; word-wrap: break-word;'>${charity.type}</div>
            <div id="supplier-result-${i+1}-customer-reference" class="col-3 my-auto" style='word-break: break-all; word-wrap: break-word;'>${charity.memberNumber}</div>
            <div id="supplier-result-${i+1}-deleted" class="col-2 my-auto" style='word-break: break-all; word-wrap: break-word;'>
                ...
                <div class="button-container d-flex justify-content-end align-items-center">
                    <g:if test="${charity.active != true}">
                        <button id="toggle-supplier-deleted-button-${i+1}" class="btn btn-wl p-1 me-1" style="min-width: 80px; font-size: 0.9rem;"
                                onclick="event.stopPropagation(); toggleSupplierDeleted(${charity.id}, ${charity.active}, {offset: ${offset ?: 0}, sortColumn: '${sortParams?.sortColumn}'})">
                            Reinstate
                        </button>
                    </g:if>
                    <g:else>
                        <button id="toggle-supplier-deleted-button-${i+1}" class="btn btn-danger p-1 me-1" style="min-width: 80px; font-size: 0.9rem;"
                                onclick="event.stopPropagation(); toggleSupplierDeleted(${charity.id}, ${charity.active},{offset: 0, sortColumn: '${sortParams?.sortColumn}'})">
                            Delete
                        </button>
                    </g:else>
                </div>

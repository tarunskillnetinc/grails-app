...
%{--                <g:checkBox name="category.id" id="category-${category.id}" checked="${category.id == selectedCategoryId}" value="${category.id}" class="col-1 form-check-input wl-checkbox" onclick="${triggerOnCategoryChange ? 'onCategoryChanged('+category.id+');' : 'return;'}" />--}%
...
%{--                <g:render template="/multiSelectCategory/categorySelectInputs" model="[categories: category.childCategories, productCategoryList: productCategoryList, selectedCategoryId: selectedCategoryId, level: level + 1, triggerOnCategoryChange: triggerOnCategoryChange]" />--}%
%{--            </g:if>--}%
%{--        </div>--}%
%{--    </g:if>--}%
%{--</g:each>--}%

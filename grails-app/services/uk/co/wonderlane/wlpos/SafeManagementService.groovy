...
class SafeManagementService extends MySqlPoolDal {
    ...
    def createNewSafeSession(int retailerId, int storeId, int safeId, boolean isAutoGenerated) {
        if (!hasActiveSafeSession(safeId)) {
            populateNewSafeSession(retailerId, storeId, safeId, isAutoGenerated, null)
        }
    }
    ...
    SafeSession createNewSafeSessionWithTenderTotals(int retailerId, int storeId, int safeId, List<TenderTotal> tenderTotals, boolean isAutoGenerated) {
        if (!hasActiveSafeSession(safeId)) {
            populateNewSafeSession(retailerId, storeId, safeId, isAutoGenerated, tenderTotals)
        }
    ...
    SafeSession addTenderToSafe(int safeId, List<ReconciliationTotal> tenderAmounts) {
        ...
        String errorMsg = "Failed to add tender movements for retailer: ${springSecurityService.principal.retailerId}, store: ${springSecurityService.principal.storeId} and safeId: ${safeId}"
        log.error(errorMsg)
    ...
    def addSpotCheckAudit(SafeSession safeSession) {
        try {
            ...
            addAudit(safeSession, SafeSessionAction.SPOT_CHECK, false, loggedInUser, null, null)
        } catch (Exception ex) {
    ...
    void processDataSave(SafeSessionSaveCommand safeSessionSaveCommand, SafeSession safeSession) {
        try {
            ...
            SafeSessionAction action = safeSessionSaveCommand.isRecount ? SafeSessionAction.RECOUNT : safeSessionSaveCommand.isFinalise ? SafeSessionAction.FINALISE : SafeSessionAction.RECONCILE

            List<TenderTotal> tenderTotalList = new ArrayList<>()
            if (action == SafeSessionAction.FINALISE) {
                for (ReconciliationTotal reconciliationTotal : safeSession.getReconciliationTotals()) {
            ...
            addAudit(safeSession, action, false, loggedInUser, null, tenderTotalList)
        } catch (SafeSessionUpdateException ex) {
            throw ex
        } catch (Exception ex) {
    ...
    private SafeSession populateNewSafeSession(int retailerId, int storeId, int safeId, boolean isAutoGenerated, List<TenderTotal> tenderTotals) throws Exception {
        ...
        addAudit(safeSession, SafeSessionAction.OPEN, isAutoGenerated, loggedInUser, null, tenderTotals) // Audit for session open action

        return safeSession;
    }

    void addAudit(SafeSession safeSession, SafeSessionAction safeSessionAction, boolean isAutoGenerated, User loggedInUser, Integer tenderMovementId, List<TenderTotal> tenderTotalList){
        try {
            SafeSessionAudit safeSessionAudit = initializeSafeSessionAudit(safeSession, safeSessionAction, isAutoGenerated, loggedInUser, tenderMovementId, tenderTotalList)
    ...
    private SafeSessionAudit initializeSafeSessionAudit(SafeSession safeSession, SafeSessionAction safeSessionAction, boolean isAutoGenerated, User loggedInUser, Integer tenderId, List<TenderTotal> tenderTotalList) {
    ...
    private void saveSafeSessionAudit(SafeSessionAudit safeSessionAudit){
        try (Connection conn = getConnection(); CallableStatement saveSafeSessionStatement = conn.prepareCall("{ call saveSafeSessionAudit(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }")) {
            ...
            saveSafeSessionStatement.setString(7, safeSessionAudit.getUsersRealName())
            saveSafeSessionStatement.setTimestamp(8, commonService.convertToSqlTimestamp(safeSessionAudit.getTimestamp()))
            saveSafeSessionStatement.setString(9, (safeSessionAudit?.extras != null) ? gsonProvider.gson.toJson(safeSessionAudit.extras) : null)
            if (safeSessionAudit.getTenderMovementId()) {
                saveSafeSessionStatement.setInt(10, safeSessionAudit.getTenderMovementId())
            } else {
                saveSafeSessionStatement.setNull(10, Types.INTEGER)
            }
            if (safeSessionAudit.getTenderMovementValue() != null && safeSessionAudit.getTenderMovementValue().size() != 0) {
                saveSafeSessionStatement.setString(11, gsonProvider.gson.toJson(safeSessionAudit.getTenderMovementValue()));
            } else {
                saveSafeSessionStatement.setString(11, null);
            }
            saveSafeSessionStatement.executeUpdate();
        } catch (SQLException ex) {

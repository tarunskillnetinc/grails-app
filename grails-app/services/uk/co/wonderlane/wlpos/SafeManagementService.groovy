...
//            }
    ...
    void processDataSave(SafeSessionSaveCommand safeSessionSaveCommand, SafeSession safeSession) {
        try {
            ...
            SafeSessionAction action = command.isRecount ? SafeSessionAction.RECOUNT : command.isFinalise ? SafeSessionAction.FINALISE : SafeSessionAction.RECONCILE
            ...
            if (action == SafeSessionAction.FINALISE) {
                for (ReconciliationTotal reconciliationTotal : safeSession.getReconciliationTotals()) {
                    if (reconciliationTotal.value != BigDecimal.ZERO) {
                        TenderTotal tenderTotal = new TenderTotal(reconciliationTotal.tenderType.id, reconciliationTotal.tenderType.name, reconciliationTotal.tenderType.cashTender)
    ...
    private SafeSession populateNewSafeSession(int retailerId, int storeId, int safeId, boolean isAutoGenerated) throws Exception {
        ...
        addAudit(safeSession, SafeSessionAction.OPEN, isAutoGenerated, loggedInUser, null, null) // Audit for session open action
        return safeSession;
    }

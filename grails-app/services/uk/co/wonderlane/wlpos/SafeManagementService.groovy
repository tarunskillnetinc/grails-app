...
class SafeManagementService extends MySqlPoolDal {
    ...
    //This will load counted tenders and prepare them into move into next safe session
    List<TenderTotal> getTendersToMoveIntoNewSafeSession(SafeSession oldSafeSession) {
        Map<TenderType, TenderTotal> tenderTotalsToMove = new HashMap<>();

        for (ReconciliationTotal reconciliationTotal : oldSafeSession.getReconciliationTotals()) {
            if (reconciliationTotal.value.compareTo(BigDecimal.ZERO) > 0) {
                tenderTotalsToMove.computeIfAbsent(
                        reconciliationTotal.tenderType,
                        type -> new TenderTotal(type)
                ).value = reconciliationTotal.value
            }
        }
        return new ArrayList<>(tenderTotalsToMove.values());
    }
    ...
    boolean isSafeFinalisingWarningRequired(SafeSession safeSession){
        ...
        List<TenderTotal> tenderTotalsToMove = getTendersToMoveIntoNewSafeSession(safeSession)
        boolean isTenderAvailableToMove = isTenderAvailableToMove(tenderTotalsToMove)
        return !safe.active && isTenderAvailableToMove
    }

...
class ShiftService extends MySqlPoolDal {
    ...
    boolean isCashManagementEnable(int tillId) {
     ...
     void updateRollingFloat(Shift oldShift, int retailerId, int storeId, int tillId){
        def cashManagementConfig = cashManagementService.getCashManagementConfig(shift.getRetailerId(), shift.getStoreId())
        def newShift = getOpenShift(retailerId, storeId, tillId) //Load existing open shift
        if ((newShift == null || !(newShift.getShiftStatus() == ShiftStatus.OPEN)) && cashManagementConfig.isRollingFloatEnabled()) {
            newShift = createNewShift(retailerId, storeId, tillId, true) //call function to open shift
        }
        def oldTotal = oldShift.tenderTotals.find { it.tenderType == TenderType.CASH }
        if (cashManagementConfig.rollingFloatValue > 0) {
            moveRollingFloatToNewShift(newShift, oldTotal, cashManagementConfig)

        }

    }

    private void moveRollingFloatToNewShift(Shift newShift, TenderTotal oldTotal, CashManagementConfig cashManagementConfig){

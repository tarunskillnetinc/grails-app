    ...
    def ajaxUpdateSafeSessionReconcileData(SafeSessionCashUpCommand safeSessionCashUpCommand) {
        try {
            ...
            if (safeSession != null) {
                ...
                if (safeSession.getSessionStatus() == SafeSessionStatus.OPEN || safeSession.getSessionStatus() == SafeSessionStatus.RECONCILED) {
                    def varianceReasons = reasonCodeService.getReasonCodesByType(safeSession.getRetailerId(), ReasonCodeType.TENDER_RECONCILIATION_SAFE_VARIANCE)
        ...
        try {
            ...
            if (validStatus) {
                ...
                if (safeSessionSaveCommand.isFinalise) {
                    ...
                    boolean hasTender = tenderTotals.stream().anyMatch { it.getValue() != BigDecimal.ZERO }
                    if (safe.active || hasTender) {

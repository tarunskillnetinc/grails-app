                ...
                for (Integer tillId : tillNos) {
                    try {
                        if (!tenderMovementService.isOpenShiftAvailable(tillId)) {
                            ...
                            //add shift audit
                            tenderMovementService.updateShiftBalanceTotals(ShiftAction.ADD_FLOAT, tenderType.id, tenderType.name, tenderType.cashTender, amount, tenderMovementId, tillId)
    ...
    def processPayOut() {
        ...
        try {
            ...
            if (!isAPayOutValidAmount(amount)) {
                ...
                //update safe session tender totals
                //add safe session audit
                tenderMovementService.updateSafeSessionBalanceTotals(SafeSessionAction.CASH_LIFT, tenderType?.id, tenderType?.name, tenderType?.cashTender, amount.negate(), tenderMovementId, safeId)

...
public class RabbitMQMockChannel implements Channel {

    boolean isOpenChannel = true;

    private HashMap<String, Object> exchangeMessageMap = new HashMap<>();
    private HashMap<String, Object> queueMessageMap = new HashMap<>();

    RabbitMQMockChannel(boolean isOpenChannel){
    ...
    public void basicPublish(String exchange, String routingKey, AMQP.BasicProperties props, byte[] body) throws IOException {
        if (exchange != null && !exchange.isEmpty() && exchange.length() > 0){
            getExchangeMessageMap().put(exchange, body);
        }

        if (routingKey != null && !routingKey.isEmpty() && routingKey.length() > 0){
            getQueueMessageMap().put(routingKey, body);
        }
    ...
    public HashMap<String, Object> getExchangeMessageMap() {
        return exchangeMessageMap;
    }

    public void setExchangeMessageMap(HashMap<String, Object> exchangeMessageMap) {
        this.exchangeMessageMap = exchangeMessageMap;
    }

    public HashMap<String, Object> getQueueMessageMap() {
        return queueMessageMap;
    }

    public void setQueueMessageMap(HashMap<String, Object> queueMessageMap) {
        this.queueMessageMap = queueMessageMap;
    }
}
